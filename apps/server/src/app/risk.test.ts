...
it.each([false, true])('registered endpoint returns only a decision; kill switch %s', async (killSwitchEnabled) => {
  ...
  const app = createApp(environment, database, false, { getQuote, isConfigured: () => false }, { getPortfolio }, {
    getRiskContext: async () => ({ ...context, killSwitchEnabled }),
  });
  try {
...
it('default app fails closed without invented risk context', async () => {
...
it.each([{ ...input, quantity: 10 }, { ...input, confidence: null }, { symbol: '005930' }, { ...input, context }, { ...input, policy: {} }, { ...input, side: 'HOLD' }])('rejects malformed HTTP input before context lookup %j', async (payload) => {
  ...
  const app = createApp(environment, database, false, undefined, undefined, { getRiskContext });
  try {
...
it('returns INVALID_PROPOSAL for semantically invalid financial strings', async () => {
  const app = createApp(environment, database, false, undefined, undefined, { getRiskContext: async () => context });
  try {
...
it('sanitizes context failure without exposing state or secrets', async () => {
  const logs: string[] = [];
  const app = createApp(environment, database, { write: (chunk) => { logs.push(chunk); } }, undefined, undefined, {
    getRiskContext: async () => { throw new Error('fixture-private-account-secret'); },
  });
  try {
    const response = await app.inject({ method: 'POST', url: '/api/v1/risk/evaluate', payload: input });
