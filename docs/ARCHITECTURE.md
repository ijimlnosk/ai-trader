...
cumulative order_fills and the broker position mirror. Existing rows remain readable by old columns.
...
The deliberately limited human-operated paper execution v1 uses authoritative KIS buying power and
an environment kill switch. Daily realized P/L and loss streak still use the explicit initial paper
state, as requested for this checkpoint; they do not measure actual trading losses. See decision 0005.
...
later positions. Fill snapshots are not individual execution IDs, fee/tax accounting or realized P/L.
